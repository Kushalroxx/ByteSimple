      ...
      >
      {
        navOptions.map((option)=>{
          return(
              ...
              <Link className={`w-full font-bold bg-foreground text-background py-2 text-center rounded-2xl` + (path === `/${option.toLowerCase()}` ? " bg-secondary text-foreground" : "")} href={`/admin/${option.toLowerCase()}`} key={option}>
              {option}
            </Link>
          )
        })}
